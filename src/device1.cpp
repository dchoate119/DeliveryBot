...
void loop() {
    ...
    if (currentTime - lastLoopTime >= loopInterval) {
        ...
        // Calculate tilt angles using atan2
        angleY = atan2(accX, sqrt(accY * accY + accZ * accZ)) * 180.0 / M_PI;  // Y-axis angle in degrees
        angleX = atan2(accY, sqrt(accX * accX + accZ * accZ)) * 180.0 / M_PI;  // X-axis angle in degrees
